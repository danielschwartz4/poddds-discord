...
const cronScheduler = (client: Client<boolean>, server_id: string) => {
    ...
    for (user in users) {
        ...
        guild.members.fetch(user_id).then((user: any) => {
            if (user.roles.cache.some(role => role.name === 'podmate')) {
                ...
                let r: Role;
                let three_streak_id = guild.roles.cache.find(r => r.name === "3 days active streak!");
                let seven_streak_id = guild.roles.cache.find(r => r.name === "7 days active streak!");

                user_streak > 3 ? user.roles.add(three_streak_id) : user.roles.remove(three_streak_id);
                user_streak > 7 ? user.roles.add(seven_streak_id) : user.roles.remove(seven_streak_id);
            }
        })
    }
