...
export const autokick = async (
    ...
    const activeGoals = await WeeklyGoal.find({ where: { isActive: true, timeZone: timeZoneIsUTCMidnight }})
    ...
    users.forEach(async (userObject: User) => {
        ...
        if (user?.roles.cache.some((role) => role.name === "new member")) {
            ...
            if (timeZoneIsUTCMidnight == "-5") {
                let timeDifference = current_date.getTime() - userObject.createdAt.getTime();
        ...
        // if adjustedEndDate of weekly_goal is 7 days ago, change role of person to kicked
            ...
            let recentWeeklyGoal = await WeeklyGoal.findOne({ where: { discordId: userObject.discordId }, order: { id: 'DESC' }})
            ...
            if (recentWeeklyGoal?.timeZone == timeZoneIsUTCMidnight) {
                ...
                if (recentWeeklyGoal?.adjustedEndDate) {
                    let timeDifference = current_date.getTime() - recentWeeklyGoal?.adjustedEndDate?.getTime();
                    let daysDifference = timeDifference / (1000 * 3600 * 24);
                    if (daysDifference == 6) {
                        client.users.fetch(user.id).then((user) => {
                        user.send(
