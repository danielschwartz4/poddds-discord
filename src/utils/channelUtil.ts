...
export const createTextChannel = (
  channelName: string,
  channel_permission_overwrites: any[],
  position?: number
) => {
...
export const createGoalsLeftTodayCategory = async (
  position: number,
  type: GoalType,
  podId: number
) => {
...
export const createGoalsLeftTodayChannel = async (
  user: User,
    ...
    .then(async (goal_left_channel) => {
      if (weekly_goal?.description) {
        ...
        if (Difference_In_Days === 1) {
          days_left_message = "1 day left! 🏁 🏃‍♂️ " + `<@${user.discordId}>`;
        }
        if (Difference_In_Days < 1) {
          expiredGoalNotif(user.discordId, podType, weekly_goal);
        } // backup code in case expired goal notif doesn't trigger somehow
        ...
        (CLIENT.channels.cache.get(goal_left_channel.id) as TextChannel).send(
          // fix starting below
...
export const readPodCategoryChannelsByType = async (
  discordId: string,
  type: GoalType
) => {
...
export const readPodCategoryChannelsByPodId = async (
  podId: number,
  type: GoalType
) => {
  let podName: string;
...
export const readPodCategoryChannelByPodId = async (
  podId: number,
  type: GoalType
) => {
  let podName: string;
...
export const readPodGoalsLeftTodayCategoryChannelByPodId = async (
  podId: number,
  type: GoalType
) => {
...
export const deleteGoalLeftTodayChannel = async (
  podId: number,
  podType: GoalType,
  userId: string
) => {
  let goalsLeftCategoryChannel =
    await readPodGoalsLeftTodayCategoryChannelByPodId(podId, podType);
  const goalsLeftChannels = GUILD()?.channels.cache.filter(
  ...
  const userChannels = goalsLeftChannels?.filter(
    (c) => c.name === user?.discordUsername.toLowerCase()
  );
...
export const deleteGoalLeftTodayChannelByPodType = async (
  podType: GoalType,
  userId: string
) => {
  const user = await readUser(userId);
  ...
  let goalsLeftCategoryChannel =
    await readPodGoalsLeftTodayCategoryChannelByPodId(podId as number, podType);
  const goalsLeftChannels = GUILD()?.channels.cache.filter(
    (c) => c.parentId === goalsLeftCategoryChannel?.id
  );
...
export const deleteGoalLeftTodayChannelByChannelId = async (
  channelId: string,
  userId: string
) => {
  const channel = CLIENT.channels.cache.get(channelId) as TextChannel;

  const podType = channel?.parent?.name.includes("💪") ? "fitness" : "study";
...
export const clearOldGoalsLeftTodayChannels = async (
  podId: number,
  podType: GoalType
) => {
  let goalsLeftCategoryChannel =
  ...
  goalsLeftChannels?.forEach((guildChannel) => {
    ...
    channel_text.messages.fetch({ limit: 1 }).then((msg) => {
      ...
      if (msgTimestamp) {
        ...
        const days_elapsed =
          (TODAY().getTime() - msgTimestamp) / (1000 * 60 * 60 * 24);
        if (days_elapsed > 1) {
          console.log(
            "deleting channel that was here for 1+ days for ",
            guildChannel.name,
            " in pod ",
            podId
          );
